...
class LineService {
   ...
   * 回應已註冊會員
   */
  private async replyToRegisteredMember(
    replyToken: string,
    memberName: string,
    userMessage: string
  ): Promise<void> {
   ...
   * 回應未註冊用戶 - 提供註冊連結
   */
  private async replyToUnregisteredUser(
    replyToken: string,
    lineUserId: string
  ): Promise<void> {
    try {
            ...
            contents: [
              {
                ...
                action: {
                  type: 'uri' as const,
                  label: '🚀 立即註冊會員',
                  uri: 'https://service.peida.net/client/register',
                },
                style: 'primary' as const,
                color: '#1DB446' as const,
              },
      ...
      try {
        ...
        const fallbackMessage = {
          type: 'text' as const,
          text: `🦁 北大獅子會\n\n歡迎您！請點擊連結完成註冊：\nhttps://service.peida.net/client/register`,
        };
        await this.client.replyMessage(replyToken, fallbackMessage);
  ...
  async pushMessage(
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Push message failed',
      };
    }
  }
  ...
  async pushFlexMessage(
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Flex push message failed',
      };
    }
  }
   ...
   * 推送會員資訊
   */
  async pushMemberInfo(
    userId: string,
    memberName: string,
    memberLevel: string,
    joinDate: string
  ): Promise<LineServiceResponse> {
    const flexMessage = this.createFlexMemberCard(
      memberName,
      memberLevel,
      joinDate
    );
   ...
   * 獲取即將到來的活動
   */
  private async getUpcomingEvents(): Promise<
  ...
  async pushCheckinNotification(
    ...
    try {
      ...
      return {
        success: true,
        message: 'Checkin notification sent successfully',
      };
    } catch (error) {
