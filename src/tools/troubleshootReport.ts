...
class TroubleshootReporter {
  ...
  private addResult(category: string, status: 'pass' | 'warning' | 'error', message: string, details?: string, suggestions?: string[]) {
    this.results.push({
      category,
      status,
      message,
      ...
      files.forEach((file: string) => {
        try {
          ...
          if (content.includes('import') && !content.includes('from') && !content.includes('require')) {
            this.addResult('Source Files', 'error', `${file}: 不完整的 import 語句`, content.substring(0, 200), ['檢查 import 語法', '確認模組路徑正確']);
          }

          // 檢查未捕獲的 async/await
          const asyncMatches = content.match(/async\s+\w+/g);
          const tryMatches = content.match(/try\s*{/g);
          if (asyncMatches && asyncMatches.length > 0 && (!tryMatches || tryMatches.length === 0)) {
            this.addResult('Source Files', 'warning', `${file}: async 函數缺少錯誤處理`, undefined, ['添加 try-catch 區塊', '使用 .catch() 處理 Promise 錯誤']);
          }

          // 檢查型別錯誤（簡單檢查）
          if (content.includes('any') && content.includes('interface')) {
            this.addResult('Source Files', 'warning', `${file}: 使用 any 型別可能導致型別安全問題`, undefined, ['明確定義型別', '使用具體的介面']);
          }
          ...
          if (importMatches) {
            importMatches.forEach(importMatch => {
              const importedItems = importMatch.match(/{([^}]+)}/)?.[1]?.split(',').map(s => s.trim());
              importedItems?.forEach(item => {
                if (!content.includes(item.replace(/\s+as\s+\w+/, '').trim()) || content.split(item).length <= 2) {
                  this.addResult('Source Files', 'warning', `${file}: 可能有未使用的 import: ${item}`, undefined, ['移除未使用的 import', '檢查程式碼使用情況']);
                }
              });
            });
          }
          ...
          this.addResult('Source Files', 'error', `${file}: 讀取錯誤`, err.message, ['檢查檔案權限', '確認檔案編碼']);
        }
      });
    });
  }
  ...
  checkEnvironmentVariables() {
    ...
    try {
      ...
      this.addResult('Environment', 'error', '.env 檔案解析錯誤', err.message, ['檢查 .env 檔案格式', '確認無特殊字元']);
    }
  }
  ...
  checkFrontendFiles() {
      ...
      if (fs.existsSync(dir)) {
        ...
        htmlFiles.forEach(file => {
          try {
            ...
            if (file.includes('liff') && !content.includes('liff.init')) {
              this.addResult('Frontend', 'warning', `${file}: LIFF 檔案未包含初始化程式碼`, undefined, ['添加 liff.init() 調用', '確認 LIFF ID 設定']);
            }

          } catch (err: any) {
            this.addResult('Frontend', 'error', `${file}: 讀取錯誤`, err.message, ['檢查檔案權限']);
          }
        });
        ...
        jsFiles.forEach(file => {
          try {
            ...
            this.addResult('Frontend', 'error', `${file}: 語法錯誤`, err.message, ['檢查 JavaScript 語法', '確認模組引入正確']);
          }
        });
      }
    });
    ...
    if (!frontendFound) {
      this.addResult('Frontend', 'warning', '未發現前端檔案目錄', undefined, ['確認前端檔案位置', '檢查目錄結構']);
    }
  }
  ...
  async runHealthCheck(): Promise<void> {
    ...
    return new Promise((resolve) => {
          ...
          if (res.statusCode === 200) {
            try {
              ...
              if (healthData.models === false) {
                this.addResult('Health Check', 'error', '模型初始化失敗', undefined, ['執行 init-schema', '檢查模型定義']);
              }
            } catch (parseErr) {
              this.addResult('Health Check', 'warning', 'Health check 回應格式異常', data, ['檢查 health 端點實作']);
            }
          } else {
            this.addResult('Health Check', 'error', `Health check 失敗 (狀態: ${res.statusCode})`, data, ['檢查伺服器狀態', '確認路由設定正確']);
          }
          resolve();
        });
      });
      ...
      req.on('error', (err) => {
        this.addResult('Health Check', 'error', 'Health check 連線失敗', err.message, ['確認伺服器已啟動', '檢查埠號設定', '確認防火牆設定']);
        resolve();
      });

      req.setTimeout(5000, () => {
        req.destroy();
        this.addResult('Health Check', 'error', 'Health check 逾時', undefined, ['檢查伺服器回應時間', '確認網路連線']);
        resolve();
      });
    });
  }
  ...
  generateReport() {
    ...
    if (errorResults.length > 0) {
      ...
      errorResults.forEach((result, index) => {
        console.log(chalk.red(`${index + 1}. [${result.category}] ${result.message}`));
        if (result.details) {
    ...
    if (warningResults.length > 0) {
      ...
      warningResults.forEach((result, index) => {
        console.log(chalk.yellow(`${index + 1}. [${result.category}] ${result.message}`));
        if (result.details) {
          console.log(chalk.gray(`   詳細: ${result.details}`));
        }
        if (result.suggestions) {
    ...
    console.log(chalk.cyan('\n💡 總結建議:'));
    if (errorResults.length > 0) {
      console.log(chalk.red('1. 優先修正所有錯誤項目，這些可能導致系統無法正常運作'));
    }
    if (warningResults.length > 0) {
    ...
    fs.writeFileSync('troubleshoot_report.json', JSON.stringify(reportData, null, 2));
