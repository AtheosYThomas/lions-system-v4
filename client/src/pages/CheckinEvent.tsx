...
const CheckinEvent = () => {
  ...
  useEffect(() => {
    ...
    const performCheckin = async () => {
      try {
        ...
        // 初始化 LIFF
        setCheckinStatus({ status: 'checking', message: '正在初始化 LINE 登入...' });
        await window.liff.init({ liffId: config.liffId });

        // 檢查登入狀態
        if (!window.liff.isLoggedIn()) {
          setCheckinStatus({ status: 'checking', message: '請登入 LINE 帳號...' });
          window.liff.login();
          return;
        }

        // 獲取用戶資料
        setCheckinStatus({ status: 'checking', message: '正在取得用戶資料...' });
        ...
        // 檢查會員狀態
        setCheckinStatus({ status: 'checking', message: '正在檢查會員狀態...' });
        ...
        if (checkinResponse.ok) {
          ...
          if (checkinResult.error?.includes('已經簽到過了') || checkinResponse.status === 409) {
            setCheckinStatus({
              status: 'already',
    ...
    const initializeCheckin = async () => {
      try {
        setCheckinStatus({ status: 'checking', message: '正在載入 LIFF SDK...' });
  ...
  const getStatusColor = () => {
    switch (checkinStatus.status) {
      ...
      case 'already': return 'text-yellow-600';
      case 'error': return 'text-red-600';
      case 'not_member': return 'text-blue-600';
